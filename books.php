...
$sql = "select * from books where sale_bid=0 and category='".$_GET['cat']."' limit 0,9";
...
$sql_cat = "select category from books where sale_bid=0";

$retval_cat = mysql_query( $sql_cat, $conn );
...
while ($row = mysql_fetch_array($retval_cat, MYSQL_ASSOC)) {
   $res[] = $row['category'];
}
